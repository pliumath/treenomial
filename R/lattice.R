...
treeToLattice <- function(trees, numThreads = -1) {
  ...
  for (i in 1:length(latList)){
    h <- max(latList[[i]][,4])
    nh <- ceiling((h+1)/30)
    latList[[i]][,5:(5+nh-1)] <- lattPositions[[i]]
    colnames(latList[[i]]) <- c("node","pl","bl","depth",paste0("L", 1:nh))
  }
...
allocateLattice <- function(tree, tips){
  ...
  # L[L[,2]==0,3] <- 1
  nh <- ceiling((h+1)/30)
  BinPos <- matrix(0L, nrow = nodes, ncol = nh)
  L <- as.data.frame(cbind(ind,pl,bl,dpt,BinPos))
  L[L[,2]==0,5:(5+nh-1)] <- c(1,rep(0,nh-1))
